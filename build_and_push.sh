...
echo "Construyendo la imagen de Docker Fastapi service ..."

docker build --no-cache -t ${NAME_SERVICE}-fastapi:${CODEBUILD_RESOLVED_SOURCE_VERSION}-fastapi .

docker tag ${NAME_SERVICE}-fastapi:${CODEBUILD_RESOLVED_SOURCE_VERSION}-fastapi $AWS_ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/${IMAGE_REPO_NAME}:${CODEBUILD_RESOLVED_SOURCE_VERSION}-fastapi

docker push $AWS_ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/${IMAGE_REPO_NAME}:${CODEBUILD_RESOLVED_SOURCE_VERSION}-fastapi
